...
pub trait PaymentsPayoutApi: Send + Sync {
    async fn create_payout(&self,  params: CreatePayoutParams ) -> Result<models::PayoutResponse, Error<CreatePayoutError>>;
    async fn execute_payout_action(&self,  params: ExecutePayoutActionParams ) -> Result<models::DispatchPayoutResponse, Error<ExecutePayoutActionError>>;
    async fn get_payout(&self,  params: GetPayoutParams ) -> Result<models::PayoutResponse, Error<GetPayoutError>>;
}
...
impl PaymentsPayoutApi for PaymentsPayoutApiClient {
    ...
    async fn execute_payout_action(&self,  params: ExecutePayoutActionParams ) -> Result<models::DispatchPayoutResponse, Error<ExecutePayoutActionError>> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<ExecutePayoutActionError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
