...
pub trait StakingApi: Send + Sync {
    ...
    async fn execute_action(&self,  params: ExecuteActionParams ) -> Result<models::ExecuteActionResponse, Error<ExecuteActionError>>;
...
impl StakingApi for StakingApiClient {
    ...
    async fn execute_action(&self,  params: ExecuteActionParams ) -> Result<models::ExecuteActionResponse, Error<ExecuteActionError>> {
        ...
        let local_var_uri_str = format!("{}/staking/chains/{chainDescriptor}/{actionId}", local_var_configuration.base_path, chainDescriptor=crate::apis::urlencode(chain_descriptor), actionId=crate::apis::urlencode(action_id));
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<ExecuteActionError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
