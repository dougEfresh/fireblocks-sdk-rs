...
///   allow multi-input and multi-output transfers. All other blockchains allow transfers
///   with one source address and one destination address. * `MINT` - Mints new tokens.
///   Supported for Stellar, Ripple, and EVM-based blockchains. * `BURN` - Burns
...
///   - An off-chain message in either Ethereum Personal Message or EIP712 format.
///   Use it to sign specific readable messages that are not actual transactions.
///   [Learn more about typed messages](https://developers.fireblocks.com/docs/typed-message-signing).
///   * `RAW` - An off-chain message with no predefined format. Use it to sign any
///   message with your private key, including protocols such as blockchains and
///   custom transaction types that Fireblocks do not natively support. [Learn more
///   about raw signing transactions.](https://developers.fireblocks.com/docs/raw-message-signing)
...
///   transaction is automatically created when adding assets on these blockchains
///   at a vault account. * `STAKE` - Assign assets to a staking pool managed by
///   a staking validator. Supported for Stellar and EVM-based blockchains. This
///   transaction is automatically created when performing staking operations. *
///   `UNSTAKE` - Remove assets from a staking pool managed by a staking validator.
///   Supported for Stellar and EVM-based blockchains. This transaction is automatically
///   created when performing staking operations. * `WITHDRAW` - Transfer assets
///   from a dedicated staking vault account to another address. Supported for Stellar
///   and EVM-based blockchains. This transaction is automatically created when performing
///   staking operations.      **Note:** Fireblocks will rename this type from `WITHDRAW`
///   to a different type name soon. There will be a 7-day notice regarding the new
///   type name.  * `SUPPLY_TO_COMPOUND` - Deprecated since April 1st, 2023. Older
///   transactions may have this as their operation, in case users in the workspace
///   have used the direct integration between Fireblocks and the Compound DeFI protocol.
///   * `REDEEM_FROM_COMPOUND` - Deprecated since April 1st, 2023. Older transactions
...
///   `APPROVE` - Enables the approve function for a smart contract to withdraw from
///   a designated wallet. [Learn more](https://support.fireblocks.io/hc/en-us/articles/4404616097426-Approve-Transaction-Amount-Cap).
